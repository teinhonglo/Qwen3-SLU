...
def normalize_nbest(nbest: Any) -> List[str]:
    if nbest is None:
        return []
    if isinstance(nbest, str):
        return [nbest] if nbest.strip() else []
    ...
    for hyp in nbest:
        if isinstance(hyp, str):
            ...
            text = hyp.get("text", hyp.get("pred_raw", hyp.get("hyp", "")))
        else:
            text = ""
        text = str(text).strip()
        if text:
...
def make_preprocess_fn_prefix_only(processor):
    def _preprocess(ex: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "audio": ex["audio"],
            "target": ex["text"],
            "nbest": normalize_nbest(ex.get("nbest", [])),
...
@dataclass
class DataCollatorForQwen3ASRMMI:
    processor: Any
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for feature in features:
            negatives = normalize_nbest(feature.get("nbest", []))
            if not negatives:
                raise ValueError(
                    "MMI training requires each example to contain a non-empty nbest list "
                    f"for audio={feature.get('audio', '')}"
                )

            candidates = [feature["target"]] + negatives
        ...
        full_inputs["mmi_group_sizes"] = torch.tensor(group_sizes, dtype=torch.long)
        return full_inputs
...
class MMITrainer(CastFloatInputsTrainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        group_sizes = inputs.pop("mmi_group_sizes")
        ...
        for group_size in group_sizes.tolist():
            group_scores = seq_log_probs[offset:offset + group_size]
            numerator = group_scores[0]
            denominator = torch.logsumexp(group_scores[1:], dim=0)
...
def parse_args():
    p = argparse.ArgumentParser("Qwen3-ASR MMI discriminative finetuning")
    ...
    p.add_argument("--init_model_dir", type=str, default="",
                   help="Optional source experiment/checkpoint to initialize MMI training from")
...
def main():
    ...
    if init_model_dir:
        print(f"[init] initialize MMI training from: {init_model_dir}")
    ...
    if not args_cli.train_file:
        raise ValueError("TRAIN_FILE is required (json/jsonl). Needs fields: audio, text, nbest, optional prompt")
    ...
    # source adapter checkpoint. This keeps MMI continuation consistent with
    ...
    keep = {"prompt", "audio", "target", "nbest", "prefix_text"}
    ...
    collator = DataCollatorForQwen3ASRMMI(processor=processor, sampling_rate=sr)
    ...
    if train_conf is not None and trainer.args.process_index == 0:
        ...
        with open(os.path.join(training_args.output_dir, "mmi_init_info.json"), "w", encoding="utf-8") as f:
