...
def parse_args():
    ...
    p.add_argument("--eval_file", required=True); p.add_argument("--teacher_checkpoint", default="",
        help="Optional override; by default use model_args.teacher.checkpoint_path from train_conf")
...
def run(quantized=False):
    ...
    if not args.teacher_checkpoint:
        raise ValueError("Teacher checkpoint is not configured. Set teacher.checkpoint_path in train_conf "
                         "or pass --teacher_checkpoint")
