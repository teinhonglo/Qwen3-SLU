...
def test_both_experiment_configs_use_the_requested_teacher_checkpoint():
    expected = "exp/macslu_pruneslu_teacher/macslu_qwen3_asr_17b_pruneslu_teacher/checkpoint-best"
    configs = [
    ...
    for config_path in configs:
        ...
        assert config[1]["teacher"]["checkpoint_path"] == expected
        command = (
            "source local/macslu_distillation_lib.sh; "
            f"teacher_setting_from_conf '{config_path}' checkpoint_path"
        )
        result = subprocess.run(["bash", "-c", command], check=True, text=True,
                                capture_output=True)
...
def test_distillation_runner_contains_complete_pruneslu_flow():
    ...
    expected_stages = [
        ...
        "Build teacher with the selected vocabulary mode and supervised fine-tuning",
...
def test_runtime_conf_toggles_vocabulary_pruning(tmp_path):
    ...
    for enabled in ("true", "false"):
        ...
        subprocess.run([
            ...
            "--teacher_checkpoint", "teacher/checkpoint-best",
            "--teacher_exp_dir", "teacher",
        ], check=True)
        config = json.loads(output.read_text())
        assert config[1]["vocabulary_pruning"]["enabled"] is (enabled == "true")
        assert config[1]["teacher"]["checkpoint_path"] == "teacher/checkpoint-best"
