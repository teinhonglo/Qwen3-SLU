...
if [ "$stage" -le 0 ] && [ "$stop_stage" -ge 0 ]; then
    echo "Stage 0: Prepare multi-prompt MAC-SLU jsonl from fixed jsonl"
    ...
    python local/prepare_macslu_multiprompt_jsonl.py \
...
if [ "$stage" -le 1 ] && [ "$stop_stage" -ge 1 ]; then
    echo "Stage 1: Finetuning on multi-prompt MAC-SLU"
