...
def collect_training_slot_type_counts(
    train_rows: Sequence[Dict[str, Any]],
) -> Dict[str, Counter]:
    """Count explicit, implicit, and combined slot-type occurrences in training data."""
    counts = {
        "explicit": Counter(),
        "implicit": Counter(),
        "all": Counter(),
    ...
    for row in train_rows:
        for frame in get_semantics(row, "semantics"):
            if not isinstance(frame, dict):
                continue
            for category, slot_key in (
                ("explicit", "slots"),
                ("implicit", "implicit_slots"),
            ):
                slots = frame.get(slot_key, {})
                if not isinstance(slots, dict):
                    continue
                for raw_slot_type in slots:
                    slot_type = re.sub(r"[^\w\s]", "", clean_label(raw_slot_type).lower()).strip()
                    if slot_type:
                        counts[category][slot_type] += 1
                        counts["all"][slot_type] += 1
    return counts


def plot_slot_type_metrics(
    metrics: Dict[str, Dict[str, float]],
    training_counts: Counter,
    output_stem: str,
    title: str,
) -> None:
    """Plot slot-type PRF bars and gold counts in raster and vector formats."""
    if not metrics:
        return
    ...
    train_support_counts = [training_counts.get(slot_type, 0) for slot_type in slot_types]
    width = 0.25
    figure_width = max(12.0, min(80.0, len(slot_types) * 0.65))
    ...
    count_ax.set_ylabel("Slot count")
...
def plot_slot_type_metric_charts(output_dir: str, train_file: str) -> None:
    """Render all slot-type charts from metrics generated by local/metrics.py."""
    metrics_path = os.path.join(output_dir, "slot_type_metrics.json")
    ...
    plot_specs = (
        ("all", "slot_type_prf1", "Slot Type Precision, Recall, and F1"),
        (
            "explicit",
            "explicit_slot_type_prf1",
            "Explicit Slot Type Precision, Recall, and F1",
        ),
        (
            "implicit",
            "implicit_slot_type_prf1",
            "Implicit Slot Type Precision, Recall, and F1",
        ),
    )
    for category, filename, title in plot_specs:
        plot_slot_type_metrics(
            metrics_by_category.get(category, {}),
            training_counts[category],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot MAC-SLU evaluation charts.")
    ...
    parser.add_argument("--train_file", required=True, help="Training JSONL used to count slot-type occurrences.")
    ...
    parser.add_argument("--output_dir", required=True, help="Directory for plot, CSV, and report outputs.")
    parser.add_argument("--skip_plots", action="store_true", help="Write CSV/report outputs without rendering plots.")
    parser.add_argument(
    ...
    if args.skip_plots:
        print("Skipped plot rendering because --skip_plots was set")
    else:
